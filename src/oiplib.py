...
class Particle:
  def __init__(self, func: function, x0: list, aCognitive: float = 2, aSocial: float = 2, inertia: float = 1):
  ...
  def update(self, globalBest: list):
    ...
    try:
      ...
      for i in range(self.dimension):
        ...
        vNext.append(self.inertia * self.v[i] + self.aCognitive * (self.best[i] - self.x[i]) * r1 + self.aSocial * (globalBest[i] - self.x[i]) * r2)
      ...
      print("WARN: Dimensions of global best must match amount of parameters to be optimized.")
      raise IndexError
...
class ParticleSwarm:
  def __init__(self, func: function, coordsMin: list, coordsMax: list, populationSize: int = 50, initStrategy: str = "random"):
    try:
      ...
      print("WARN: Dimensions of boundary minimum and boundary maximum must be the same.")
      raise IndexError
